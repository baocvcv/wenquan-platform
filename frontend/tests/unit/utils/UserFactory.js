...
class UserFactory {
  ...
  login(wrapper, user_group) {
    if (wrapper.vm.$store.state.user) wrapper = this.log_out(wrapper);
    let user;
    if (user_group === "Student") user = this.create_anonymous_student();
    else if (user_group === "Admin") user = this.create_anonymous_admin();
