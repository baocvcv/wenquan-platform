<template>
  <div class="treeview">
    ...
    <v-btn
      ...
      >Submit</v-btn
    >
    <v-btn v-if="edit" @click="cancel" color="error" class="mx-2 my-2"
      >Cancel</v-btn
    >
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    addSubNode() {
      ...
      if (
        ...
        this.currentSelection[0].subnodes
      ) {
        let newNode = {
          id: -1,
          name: "No name",
    ...
    removeNode() {
      ...
      if (this.currentSelection.length > 0) {
        let travelDeleteNode = item => {
          if (item.id != -1) this.deletedID.push(item.id);
          if (item.subnodes) item.subnodes.forEach(travelDeleteNode);
        };
        ...
        let removeFunc = (item, index, arr) => {
          ...
          if (item.subnodes) item.subnodes.forEach(removeFunc);
        };
  ...
  data: function() {
    return {
      ...
      //tree properties
      drag_drop: {
        ...dragndrop.selection(() => this.treeData, m => (this.treeData = m))
      },
      strategies: {
        selection: ["multiple"],
